...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{TSLA_fat_tail.png}
    \caption{Fat Tail Demo for Tesla Stock}
...
\begin{figure}[H]
    ...
    \caption{Error Scaling for Poisson Method}
    \label{fig:placeholder}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Convergence for Poisson Method}
    \label{fig:placeholder}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{accuracy_comp.png}
    \caption{Poisson vs Gaussian Accuracy}
    \label{fig:placeholder}
\end{figure}
...
\subsection*{Execution Context (Simulator vs Hardware)}
All quantum routines in this project are executed on simulators via the Classiq SDK, not on physical quantum hardware. Accessing real hardware would incur usage charges and requires explicit backend selection, so our results reflect simulated execution with shot noise rather than device noise.
...
We perform a grid search over $\epsilon$ and the IQAE confidence parameter to tune hyperparameters, then reuse the best pair for the shots-scaling experiment. We also run a separate epsilon-scaling experiment with a fixed shot budget to highlight the expected $O(1/\epsilon)$ query scaling. For this experiment, we perform another grid search over the shot count and the confidence parameter alpha and select the best possible value. The plots below are placed immediately after the relevant discussion.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{iqae_shots_alpha.png}
    \caption{Hyperparameter tuning: grid search over shot number and confidence parameter.}
    \label{fig:iqae_hyperparam_tuning}
\end{figure}
\FloatBarrier

\subsection{Additional IQAE Diagnostics}
For completeness, we include supplementary diagnostic plots that corroborate the
expected scaling behavior beyond the primary log-log figures.

\begin{figure}[ht]
    \centering
    \includegraphics[width=1\linewidth]{linearscaling.png}
    \caption{IQAE queries vs $1/\epsilon$ on linear axes.}
    \label{fig:iqae_linear_scaling}
\end{figure}

\begin{figure}[ht]
    \centering
    \includegraphics[width=1\linewidth]{shotsscaling.png}
    \caption{IQAE error vs shots on linear axes.}
    \label{fig:iqae_shots_scaling_linear}
\end{figure}
...
\FloatBarrier
\section{Real-World Extension: Gold and S\&P 500}
\subsection{Motivation and Data}
To test whether the same scaling intuition holds on realistic market data, we extend the
analysis to real securities. We focus on gold (GC=F) as a fat-tail, downside-risk case
study and the S\&P 500 (\^GSPC) as a broad-market baseline. Gold experienced extreme
volatility and sharp drawdowns in late January 2026, making it a natural stress test for
tail-risk modeling. Daily log-returns are fit to a double-Poisson jump-diffusion model
(separate positive and negative jump processes) to capture skew and excess kurtosis. The
fitted distribution is then used for Monte Carlo VaR estimation and convergence analysis,
and the same fitted model is reused for amplitude encoding in the quantum workflow.

\subsection{Double-Poisson Jump Model}
We model returns as a diffusion component plus two independent Poisson jump processes:
one for positive jumps and one for negative jumps. This separates tail behavior and
captures the asymmetry observed in real data. The parameters
$(\mu,\sigma,\lambda_+,\lambda_-,\mu_{J+},\sigma_{J+},\mu_{J-},\sigma_{J-})$
are fitted from historical returns by thresholding outlier moves and estimating
positive and negative jump statistics separately.

\subsection{Classical Monte Carlo on Fitted Curves}
Using the fitted model, we sample returns and compute VaR at the target confidence
level. We then repeat this over a range of sample sizes to measure convergence and
error scaling, analogous to the synthetic Gaussian experiments. This provides a direct
classical baseline for real-world data and establishes a reference for the quantum
experiments on the same fitted distribution.

\subsection{Quantum Extension on Fitted Curves}
The IQAE routine is reused with a new state-preparation step: the fitted double-Poisson
distribution is discretized on a grid and encoded into amplitudes. The threshold oracle
and bisection logic remain unchanged. This allows a direct comparison of query complexity
and shot noise against the classical Monte Carlo baseline on a realistic, fat-tailed
dataset.

\subsection{Real-World Plots}
Figure~\ref{fig:real_world_multi_panel} combines the empirical fits, Monte Carlo
convergence and error scaling for both assets, along with IQAE $1/\epsilon$ query
scaling for both gold (GC=F) and the S\&P 500 (\^GSPC) on the fitted real-world
distribution. We focus on gold as the primary stress-test case because risk analysis is
explicitly about worst-case behavior; gold experienced a sharp drawdown recently, making
it a natural example where tail events actually occurred.

The real-world IQAE results mirror the synthetic comparison: query complexity scales
approximately as $O(1/\epsilon)$ even when the distribution is fitted from data rather
than analytic. This provides a consistent scaling signal across both idealized and
realistic settings.

\subsection{Reproducibility Note}
Our quantum experiments use the Classiq SDK for synthesis and execution. The SDK sends
queries to Classiq's backend, which runs the circuits on a simulator; there is no
physical quantum hardware in the loop. As a result, results are reproducible given the
same model parameters and shot budgets, but require network access and valid Classiq
credentials for the synthesis step.

\begin{figure}[ht]
    \centering
    \includegraphics[width=1\linewidth]{real_world_multi_panel.png}
    \caption{Real-world results: fits, Monte Carlo convergence and error scaling for GC=F and \^GSPC, plus IQAE query scaling for both assets.}
    \label{fig:real_world_multi_panel}
\end{figure}

\section{CVaR Extension}
Beyond VaR, we also study Conditional Value at Risk (CVaR), or Expected Shortfall,
which measures the average loss in the tail beyond the VaR threshold. CVaR is more
informative for extreme events and is a common risk metric in practice.

We estimate CVaR with Monte Carlo sampling under the same Gaussian return model used
earlier, and analyze convergence and error scaling across sample sizes. We also run
an IQAE-based CVaR routine by using amplitude estimation to infer tail probability and
mapping it to CVaR, allowing us to test the same scaling rules in the CVaR setting.

\begin{figure}[ht]
    \centering
    \includegraphics[width=1\linewidth]{cvar_multi_panel.png}
    \caption{CVaR results: Monte Carlo convergence and error scaling, plus IQAE query and shot scaling.}
    \label{fig:cvar_multi_panel}
\end{figure}

\FloatBarrier
\section{Conclusion}
We implemented classical Monte Carlo VaR and quantum IQAE VaR on both synthetic and
realistic distributions, and observed the expected scaling behavior across settings.
Classically, error decays as $O(1/\sqrt{N})$ with samples; in the quantum pipeline,
IQAE shows the asymptotic $O(1/\epsilon)$ query scaling while shot noise still follows
$O(1/\sqrt{\text{shots}})$. The jump-diffusion and double-Poisson fits emphasize that
modeling assumptions and discretization dominate errors in real data, while IQAE
primarily reduces estimation error per query. The real-world extension (gold and
S\&P 500) preserves the same scaling signature on fitted, fat-tailed data, and the
CVaR extension shows that both classical and IQAE workflows carry over to tail-average
risk metrics.

\subsection*{Future Work}
There are several clear next steps. First, extend the IQAE real-world experiments to
additional asset classes and stress periods to test robustness across
market regimes. Second, refine state preparation to reduce discretization error,
including adaptive grids and higher-resolution tail modeling. Third, compare alternative
heavy-tail models (e.g., Student-$t$, EVT-based fits) against the double-Poisson
baseline and evaluate their impact on VaR and CVaR. Finally, explore execution on
actual quantum hardware backends when available to quantify the gap between idealized
simulators and noisy devices.
